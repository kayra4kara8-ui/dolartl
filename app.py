        ...
        if items:
            ...
            for it in items:
                ...
                if tarih and deger and str(deger).strip() not in ("", "None", "ND"):
                    chunk_records.append({"Tarih": tarih, "Dolar_Kuru": deger})
            all_records.extend(chunk_records)

            # Gerçekte gelen son tarihe göre ilerle (EVDS bazen daha az döndürür)
            if chunk_records:
                last_tarih = pd.to_datetime(chunk_records[-1]["Tarih"], format="%d-%m-%Y", errors="coerce")
                if pd.notna(last_tarih):
                    chunk_start = last_tarih + timedelta(days=1)
                    continue
