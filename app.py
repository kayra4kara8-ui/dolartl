...
.upload-box { background: #0d1220; border: 1px dashed #1e2d4a; border-radius: 12px; padding: 60px 40px; text-align: center; margin: 40px auto; max-width: 600px; }
.upload-icon { font-size: 3rem; margin-bottom: 16px; }
.upload-title { font-family: 'DM Mono', monospace; font-size: 1rem; color: #4a9eff; margin-bottom: 8px; }
...
def safe_ticks(vmin, vmax, n=8, decimals=2, suffix='', is_kur=False):
    ...
    Döndürür: (tickvals_list | None, ticktext_list | None)
    """
    try:
        vmin = float(vmin)
        vmax = float(vmax)
...
def yt(tv, tt, extra=None):
    """yaxis tick dict — güvenli birleştirici."""
    d = dict(extra or {})
    if tv:
        d["tickvals"] = tv
        d["ticktext"] = tt
...
PLOTLY_BASE = dict(
    ...
    legend=dict(bgcolor='rgba(13,18,32,0.8)', bordercolor='#1e2d4a', borderwidth=1, font=dict(size=11)),
...
def veri_isle(raw):
    ...
    df['Dolar_Kuru'] = df['Dolar_Kuru'].apply(lambda x: parse_sayi(x) if isinstance(x, str) else x)
    df['Dolar_Kuru'] = pd.to_numeric(df['Dolar_Kuru'], errors='coerce')
    df = df.dropna(subset=['Dolar_Kuru'])

    # Binlik nokta yanlış parse düzeltme
...
with st.sidebar:
    ...
    st.markdown("""<div style="font-family:'DM Mono',monospace;font-size:0.65rem;text-transform:uppercase;
        letter-spacing:0.15em;color:#1b6cf2;padding:20px 0 12px 0;border-bottom:1px solid #1e2d4a;">
        ◈ Veri Kaynağı</div>""", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    uploaded_file   = st.file_uploader("Excel Dosyası (.xlsx)", type=['xlsx','xls'])
    st.markdown("""<div style="font-size:0.72rem;color:#3a5070;margin-top:8px;line-height:1.7;">
...
if uploaded_file is None:
    st.markdown("""<div class="upload-box">
        ...
        <div class="upload-desc">Sol panelden .xlsx dosyanızı seçin<br><br>
            Beklenen format: <strong>EVDS</strong> sayfası<br>
            Tarih: <strong>DD-MM-YYYY</strong> · Kur: <strong>TP_DK_USD_A_YTL</strong></div>
    </div>""", unsafe_allow_html=True)
    st.stop()
