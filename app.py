...
def parse_sayi(s: Any) -> str:
    ...
    if pd.isna(s):
        return str(s)
    s = str(s).strip()
    # Türkçe/İngilizce format ayrımı
    if '.' in s and ',' in s:
        if s.rfind('.') > s.rfind(','):
            s = s.replace(',', '')
        else:
...
def veri_isle(raw_data: bytes) -> Tuple[pd.DataFrame, int]:
    ...
    Args:
        raw_data: Excel dosyasının binary içeriği
        
    Returns:
        Tuple: (işlenmiş DataFrame, temizlenen satır sayısı)
    """
    try:
...
def forward_analysis(df: pd.DataFrame, threshold: float, periods: List[int]) -> Dict[int, Dict]:
    ...
    Args:
        ...
        threshold: Sıçrama eşiği (%)
        periods: Analiz edilecek gün periyotları
        
    Returns:
        Dict: Periyot bazlı analiz sonuçları
    """
...
def create_weekly_chart(hf: pd.DataFrame, esik: float, yon: str, 
    ...
    if yon == "Yalnız Pozitif ↑":
        ...
        hf_sic = hf[hf['HaftaDegisim'].abs() >= esik].copy()
    
    hf_pos_sc = hf_sic[hf_sic['HaftaDegisim'] > 0].copy()
    hf_neg_sc = hf_sic[hf_sic['HaftaDegisim'] < 0].copy()
    
    # Etiketler
    def add_labels(sub, is_pos):
        if len(sub) == 0:
            sub['_lbl'] = pd.Series(dtype=str)
            return sub
        
        thr = sub['HaftaDegisim'].quantile(1 - etiket/100) if is_pos else \
              sub['HaftaDegisim'].quantile(etiket/100)
        
        sub['_lbl'] = sub.apply(
            lambda r: f"{r['Aralik']} {tr_fmt_pct(r['HaftaDegisim'], 1)}"
            if (is_pos and r['HaftaDegisim'] >= thr) or \
               (not is_pos and r['HaftaDegisim'] <= thr) else "",
            axis=1
        )
        return sub
    
    hf_pos_sc = add_labels(hf_pos_sc, True)
    hf_neg_sc = add_labels(hf_neg_sc, False)
...
def main():
    ...
    # Sidebar'ı render et ve parametreleri al
    (esik, gosterim_sec, yon, etiket_quantile, haftalik_esik, 
     haftalik_etiket, haftalik_yon, gun_filtre, fwd_threshold, uploaded_file) = render_sidebar()
    ...
    with tab1:
        render_tab1(df, sicramalar, pos_j, neg_j, esik, gosterim_sec, etiket_quantile)
    
    with tab2:
        render_tab2(df, hf_global, gun_filtre, haftalik_esik, haftalik_etiket, haftalik_yon)
